...
if (!gitReposDirPath || !startDate || !endDate || !author || !outRepo) {
  throw new Error('Please provide all arguments: gitReposDirPath, startDate, endDate, author, outPath')
}
...
const gitRepos = execSync(`find ${gitReposDirPath} -name .git -type d -prune`, { encoding: 'utf8' }).split('\n').filter(Boolean).map(repo => repo.replace('/.git', ''))
...
const executeCommit = (commitDate, commitMessage) => {
  ...
  if (!existsSync(contributionsFile)) execSync(`touch ${contributionsFile}`, { cwd: currentRepoDirPath })
  execSync(`echo "${commitMessage}" >> ${contributionsFile}`, { cwd: currentRepoDirPath })
  execSync(`git add .`, { cwd: currentRepoDirPath })
  execSync(`GIT_COMMITTER_DATE="${commitDate}" git commit --date="${commitDate}" -m "${commitMessage}"`, { cwd: currentRepoDirPath })
}

gitRepos.forEach(repo => {
  const gitContributions = execSync(`git log --pretty=format:"%ad - %an: %s" --after="${startDate}" --until="${endDate}" --author="${author}"`, { cwd: repo }).toString()
